...
def main():
    ...
    parser.add_argument('--validation_dir', type=str, default='wiki_demo')
    parser.add_argument('--finetune_dir', type=str, default='squad')
    ...
    validation_dir_s3_addr = f's3://{args.bucket_name}/{args.validation_dir}'
    finetune_dir_s3_addr = f's3://{args.bucket_name}/{args.finetune_dir}'
    ...
    s3_input_train = sagemaker.inputs.TrainingInput(s3_data=train_dir_s3_addr)
    s3_input_validation = sagemaker.inputs.TrainingInput(s3_data=validation_dir_s3_addr)
    s3_input_finetune = sagemaker.inputs.TrainingInput(s3_data=finetune_dir_s3_addr)
    estimator.fit({"training": s3_input_train,
                   "validation": s3_input_validation,
                   "finetune": s3_input_finetune})
